 ...
 * - Import the hashlist (64 items) into the backend DB (non-destructive)
...
export const SpikesAdminSection: React.FC<{ adminAddress: string }> = ({ adminAddress }) => {
  ...
  const loadSupply = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase}/api/spikes/status`);
  ...
  const downloadHashlist = async () => {
    setStatus(null);
    try {
      const res = await fetch(`${apiBase}/api/spikes/admin/hashlist?adminAddress=${encodeURIComponent(adminAddress)}`);
      if (res.ok) {
        const data = await res.json();
        downloadJson(data, 'spikes-hashlist.json');
  ...
  const downloadOrders = async () => {
    setStatus(null);
    try {
      const res = await fetch(`${apiBase}/api/spikes/admin/orders?adminAddress=${encodeURIComponent(adminAddress)}`);
      if (res.ok) {
        const data = await res.json();
        downloadJson(data, 'spikes-orders.json');
  ...
  const syncMarketplace = async () => {
    setStatus('⏳ Syncing Spikes into marketplace (incl. live owners)…');
    try {
      const res = await fetch(`${apiBase}/api/spikes/admin/sync-marketplace`, {
        method: 'POST',
  ...
  const runImport = async (raw: string) => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/spikes/admin/import-hashlist`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="mt-6 bg-gray-900 border-2 border-cyan-500/60 rounded-lg p-4">
      <h3 className="text-lg font-bold text-cyan-400 mb-1 flex items-center gap-2">⚡ Spikes</h3>
      <p className="text-xs text-gray-400 mb-3">
        Hashlist import · Mintlist download · Marketplace sync
        {supply && <span className="ml-2 text-cyan-300">· {supply.minted}/{supply.total} minted · {supply.active ? 'LIVE' : 'dormant'}</span>}
      </p>

      <div className="flex flex-wrap gap-2 mb-4">
        <button onClick={downloadHashlist} className="px-3 py-2 bg-cyan-600 hover:bg-cyan-700 text-black rounded text-sm font-semibold">📥 Hashlist</button>
        <button onClick={downloadOrders} className="px-3 py-2 bg-cyan-600 hover:bg-cyan-700 text-black rounded text-sm font-semibold">📥 Orders / Mintlist</button>
