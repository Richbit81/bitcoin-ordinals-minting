...
export const SmilePage: React.FC = () => {
  ...
  const handleMint = async (item?: CollectionItem) => {
    ...
    if (ordinalsAccount) {
      ...
      console.log(`[SmilePage] ✅ ${walletState.walletType?.toUpperCase()} - Verwende Ordinals-Adresse (Taproot) für Inscription:`, userAddress);
    } else {
      ...
      console.warn(`[SmilePage] ⚠️ ${walletState.walletType?.toUpperCase()} - Keine Taproot-Adresse! Verwende ${addressType}:`, userAddress);
    }
    
    // Zeige Payment-Adresse (falls vorhanden)
    const paymentAccount = walletState.accounts.find(acc => acc.purpose === 'payment');
    if (paymentAccount) {
      console.log(`[SmilePage] 💰 ${walletState.walletType?.toUpperCase()} - Payment kommt von:`, paymentAccount.address);
    }
    ...
    try {
      if (itemToMint.type === 'delegate') {
        ...
        if (selectedCollection.price && selectedCollection.price > 0) {
          ...
          try {
            if (walletState.walletType === 'unisat') {
              await sendBitcoinViaUnisat(adminAddress, selectedCollection.price);
            } else {
              await sendBitcoinViaXverse(adminAddress, selectedCollection.price);
            }
            console.log(`[SmilePage] ✅ Price paid: ${selectedCollection.price} BTC to ${adminAddress}`);
          } catch (priceError: any) {
            console.error('[SmilePage] Price payment error:', priceError);
        ...
        if (prepareData.requiresSigning && prepareData.psbtBase64) {
          ...
          console.log(`[SmilePage] Owner address (input): ${ownerAddress}`);
          console.log(`[SmilePage] User address (recipient): ${userAddress}`);
          ...
          if (ownerAddress && ownerAddress !== userAddress) {
            console.warn(`[SmilePage] ⚠️ Owner address (${ownerAddress}) differs from user address (${userAddress})`);
            console.warn(`[SmilePage] ⚠️ The user's wallet may not be able to sign this PSBT if it doesn't control ${ownerAddress}`);
          }
