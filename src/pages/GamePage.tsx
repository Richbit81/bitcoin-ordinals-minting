...
export const GamePage: React.FC = () => {
  ...
  const getNextStepHint = (): string => {
    if (!isPlayerTurn) return `Opponent turn (${gameState.phase.toUpperCase()}) - wait`;
    if (gameState.phase === 'draw') return 'Drawing card...';
    if (gameState.phase === 'main') {
      if (playableCardsCount > 0) {
        return `Play cards (${playableCardsCount} playable) or end MAIN phase`;
      }
      return 'No playable cards - click "End Main Phase"';
    }
    if (gameState.phase === 'attack') return 'Attacks resolve automatically';
    return 'End phase effects are resolving';
  };
  ...
  const handleCardClick = (card: GameCard) => {
    ...
    if (card.type === 'action' || card.type === 'status') {
      // Nur echte manuelle Zielauswahl öffnet das Target-Modal
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4">
      ...
      <div className="max-w-[1600px] mx-auto mb-4">
        ...
        <div className="mt-3 rounded-lg border border-cyan-500/40 bg-cyan-900/20 px-3 py-2 text-sm text-cyan-200">
          <span className="font-semibold">Next step:</span> {getNextStepHint()}
        </div>
      </div>
